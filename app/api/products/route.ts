...
export async function GET(request: NextRequest) {
  try {
    ...
    if (countError || dataError) {
      console.error("Database query error:", countError || dataError)
      throw new Error(countError?.message || dataError?.message || "Database query failed")
    }

    console.log("Raw products data:", data?.length || 0, "items")
    ...
    console.log("Processed products:", processedProducts.length, "Total count:", totalCount)
